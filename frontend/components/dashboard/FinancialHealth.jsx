...
const FinancialHealth = ({ latestApp }) => {
  ...
  let monthlyEmi = derived.proposed_emi;
  let dti = derived.dti_ratio;
  let maxLimit = derived.max_loan_eligibility;

  // Dynamic fallback calculations instead of static variants
  if (latestApp && !monthlyEmi && latestApp.loanAmount && latestApp.tenure) {
      const P = parseFloat(latestApp.loanAmount) || 0;
      const R = (10.5 / 100) / 12; // Base fallback rate
      const N = parseInt(latestApp.tenure) || 36;
      monthlyEmi = Math.round((P * R * Math.pow(1 + R, N)) / (Math.pow(1 + R, N) - 1));
  } else if (!monthlyEmi) {
      monthlyEmi = 0;
  }

  if (latestApp && !dti && latestApp.income && monthlyEmi) {
      dti = Math.round((monthlyEmi / (parseFloat(latestApp.income) / 12)) * 100);
  } else if (!dti) {
      dti = 0;
  }

  if (latestApp && !maxLimit && latestApp.income) {
      const annualIncome = parseFloat(latestApp.income) || 0;
      maxLimit = Math.round(annualIncome * 2.5); // Eligible up to 2.5x annual income
  } else if (!maxLimit) {
      maxLimit = 0;
  }

  const riskTier = latestApp?.risk_level || (score > 750 ? "Tier-1 prime hub" : score > 650 ? "Standard sovereign" : "Restricted tier");
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-stretch animate-in fade-in duration-700 font-sans">
      ...
      <div className="lg:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-6">
        ...
        <MetricCard 
          ...
          value={maxLimit > 0 ? `₹${(maxLimit / 100000).toFixed(1)} lac` : 'Pending'} 
          icon={ShieldCheck} 
          color="bg-blue-50" 
          iconColor="text-blue-600"
          subtext={maxLimit > 0 ? "Predictive credit limit" : "Submit application"}
        />
        <MetricCard 
          label="Utilization ratio" 
          value={dti > 0 ? `${dti}%` : 'Pending'} 
          icon={TrendingUp} 
          color="bg-green-50" 
          iconColor="text-green-600"
          subtext={dti > 0 ? "Optimal debt balance" : "Awaiting sync"}
        />
        <MetricCard 
          label="Proposed payment" 
          value={monthlyEmi > 0 ? `₹${monthlyEmi.toLocaleString()}` : 'Pending'} 
          icon={IndianRupee} 
          color="bg-slate-50" 
